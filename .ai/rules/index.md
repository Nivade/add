...
| `backend/compose.yaml`, `backend/.env*`, `backend/vite.config.ts`, `package.json`, `packages/shared/**`, `mobile/**` | [toolchain.md](toolchain.md) — Redis only resolves inside Compose; `compose.yaml` is hand-edited and `devtools:sync` reverts it; the two frontends share types, never components; debug tooling is a second container and Bugsink's DSN host is not the browser's. |
