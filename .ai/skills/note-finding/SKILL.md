...
5. Append to `.ai/findings.md`, creating it with a one-line header if it
   doesn't exist yet.
...
Read `.ai/findings.md` before starting related work if the task touches
...
fold it into `.ai/plans/` as work to do and delete the line, record it as a
rule with Boost's `record-rule` if it's a durable fact, or delete it if it no
longer applies. This repo has no remote and no issue tracker, so there is no
`gh issue create` step. This skill only appends; it never
triages unprompted.
