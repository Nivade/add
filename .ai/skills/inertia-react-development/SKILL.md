...
## Inertia v2 features, all still current in v3
...
- Reaching for axios: v3 removed it, so use the built-in XHR client or `useHttp`

## In this repo

Inertia 3 with React 19 and TypeScript, under `backend/resources/js/`.

- v3 deltas: axios is gone (`useHttp` or the built-in XHR client), `Inertia::lazy()`
  is `Inertia::optional()`, `router.cancel()` is `router.cancelAll()`, the `invalid`
  event is `httpException` and `exception` is `networkError`.
- Page props are `spatie/laravel-data` objects; import their generated types from
  `@add/shared`, never hand-write a prop type.
- URLs come from Wayfinder — `@/routes` for named routes, `@/actions` for
  controllers. `<Form {...someRoute.form()}>` over a hand-written action string.
- Layouts are chosen centrally in `resources/js/app.tsx`; a page that must suppress
  the shell returns `null` there rather than styling around it.
- `.ai/rules/product-invariants.md` binds every screen: one thing, never a list.
