...
return [
    ...
    */
    ...
    // Scoped guidelines stay off: Boost scopes them to app/**, which never matches backend/app/**.
    'rules' => [
    ...
    |--------------------------------------------------------------------------
    |
    | The agent config Boost writes belongs to the workspace root, one level up
    | from the Laravel base path it resolves against. Skills are wrapped in
    | base_path() by Boost; guidelines are not, so that one is absolute or it
    | follows the shell's working directory out of the repo.
    |
    */

    'agents' => [
        'claude_code' => [
            'guidelines_path' => base_path('../.ai/guidelines.md'),
