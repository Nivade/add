...
it('says how long the session has been running, in words rather than a countdown', function (): void {
    ...
    expect(BuildExecutionState::run($session)->elapsed)->toBe('You have just started.');

    CarbonImmutable::setTestNow('2026-09-19 09:12:00');

    expect(BuildExecutionState::run($session->refresh())->elapsed)->toBe('You have been working for 12 minutes.');
});
...
it('holds the elapsed count still while the session is paused', function (): void {
    ...
    expect(BuildExecutionState::run($session->refresh())->elapsed)->toBe('You had been working for 12 minutes.');
});
